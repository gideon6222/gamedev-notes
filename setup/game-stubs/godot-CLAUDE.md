...
A phone game for Android, built in Godot 4.7 from `C:\dev\godot-template` on {{DATE}}.
Package `{{PACKAGE}}`, repo `github.com/gideon6222/{{SLUG}}`.
...
Nothing on the web stack here. Package `{{PACKAGE}}`, APK `build/{{SLUG}}.apk`, AAB
`build/{{SLUG}}.aab`, launch component `{{PACKAGE}}/com.godot.game.GodotAppLauncher`.
