...
So the endpoint failing was **generate-release-notes**, not release creation, which the workflow
asks for with `generate_release_notes: true`. Snowball uses the identical setting and published
twice in the same minutes, so the endpoint was not down. What differed is that wildform's release
history had a **hole**: build-34 published, build-35 missing entirely, build-36 a draft. Notes are
generated by diffing against the previous release, and that is the state it could not diff. The
repair was not another build. Publishing the existing draft (`PATCH .../releases/<id>
draft=false`) skipped note generation entirely, used the APK already attached, and filled the slot
so the next build has a normal predecessor.
...
all of this. **And read the 500's body, not just its status**: the status was identical across
three runs and told us nothing, while one line of payload named the endpoint and turned a
guess about an outage into a diagnosis. A repeat 500 that another repo is not getting is not an
outage, it is state: for releases, look for a gap or a leftover draft in the history, because
`generate_release_notes` diffs against the previous release and cannot diff a hole.
